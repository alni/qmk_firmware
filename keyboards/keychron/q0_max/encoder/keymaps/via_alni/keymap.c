...
/*
    If you want to just use RGB indicators without RGB matrix effect, it is not possible 
    to disable the latter because toggling RGB off will disable everything. 

    You can workaround it with solid effect and colors off using this init function:
*/
...
layer_state_t layer_state_set_user(layer_state_t state) {
    //int curr_val = rgblight_get_val();
    /*
     switch(get_highest_layer(state)) {
        case FN:
            //rgblight_sethsv_noeeprom (HSV_RED);
            rgblight_sethsv_noeeprom (0, 255, curr_val);
            break;
        case L2:
            //rgblight_sethsv_noeeprom (HSV_GREEN);
            rgblight_sethsv_noeeprom (85, 255, curr_val);
            break;
        case L3:
            //rgblight_sethsv_noeeprom (HSV_BLUE);
            rgblight_sethsv_noeeprom (170, 255, curr_val);
            break;
        default: //  for any other layers, or the default layer
            //rgblight_sethsv_noeeprom (HSV_WHITE);
            //rgblight_sethsv_noeeprom (0, 0, curr_val);
            
            // Use the user set/saved state
            rgblight_reload_from_eeprom(); // Reload the the user saved state
            break;
    }
    */
    return state;
